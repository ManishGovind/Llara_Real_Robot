...
def pos2pix(x, y,width=500, height=250):

    x_pix = float(x) * width
    y_pix = float(y) * height
    return int(round(x_pix)), int(round(y_pix))


def image_pixel_to_robot_coords(pixel_x, pixel_y):
    pixelx_to_mm = 516/500
    pixely_to_mm = 400/250  
    
    robot_home = (-262.27, -245.77)
    pixel_home = (0,0)
    dx_mm = (pixel_x - pixel_home[0]) * pixelx_to_mm
    dy_mm = (pixel_y - pixel_home[1]) * pixely_to_mm
    robot_x = robot_home[0] + dx_mm
    robot_y = robot_home[1] - dy_mm  # subtracting since image y increases downwards
...
def action_to_text(obs_img: np.ndarray, action: dict, spatula:bool) -> str:
    ...
    px, py = pos2pix(act_pos_start[0], act_pos_start[1])
    tx, ty = pos2pix(act_pos_end[0], act_pos_end[1])
...
def query_bc(tokenizer, model, image_processor, prompt, prompt_mode, prompt_assets, action_hist, obs, detector, prompt_id):
    prepared_prompt, image_list = prepare_prompt(tokenizer, model, image_processor, prompt, mode=prompt_mode, prompt_assets=prompt_assets, spatula=False, detector=detector, uid=prompt_id)
    ...
    if 'h' in prompt_mode and len(action_hist):
        ...
        prepared_prompt += '\n'.join([f'Step {acti + 1}: ' + action_to_text(obs, acth, spatula=False) for acti, acth in enumerate(action_hist)])
    image_list.append(obs.copy())
...
def parse_bc(pick_place_point):
    p0 = parse_coordinate(pick_place_point[0])
    p1 = parse_coordinate(pick_place_point[1])
    action = {
        'pose0_position': p0,
        'pose1_position': p1,
    }
...
if __name__ == '__main__':
    ...
    eval_real_episode(args , query_bc , parse_bc)
