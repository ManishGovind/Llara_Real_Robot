...
sys.path.append('../')
from videollama3 import model_init, mm_infer, disable_torch_init
...
def model_generation(tokenizer, model, image_processor, image_list, video_list, prompt, modality='image'):
    prompt = prompt.replace('<image0>\n', '') # videollama doesnt require the token
...
def eval_episode(args, gen_action_fn, parse_action_fn):
    ...
    disable_torch_init()
    model, image_processor = model_init(
    ...
    partition_list = partition_list[:3] # filter out L4, llara doesnt use it
            ...
            for seed in seed_range:
                ...
                for step in range(args.max_length):
                    ...
                    while not action_queue and retry < 1:
                        ...
                        # cv2.imwrite(f'/users/dreilly1/Projects/VideoLLaMA3/robotics_evaluation/_frames/{step}.png', image_4_cv2)
